...
class QuarkController
{
    ...
    public function __construct()
    {
      ...
      $this->QuarkSession = new QuarkSession();
    ...
    protected function includeCssFiles()
    {
      $included = array();
      foreach($this->_css_files as $file){
        if(array_search($file, $included) === false){
          ...
          echo '<link rel="stylesheet" type="text/css" media="', $file_info[1],'"',
            ' href="', $this->QuarkUrl->baseUrl(null, true),
    ...
    protected function includeJsFiles()
    {
      ...
      $this->QuarkUrl->url('home/quark-include-js');
      echo '"></script>';
      ...
      foreach($this->_js_files as $file){
        if(array_search($file, $included) === false){
          $included[] = $file;
          echo '<script type="text/javascript" src="',
            $this->QuarkUrl->baseUrl(null, true), 'application/public/js/', $file,
            '"></script>', PHP_EOL;
        }
      }
    }
    ...
    public function setAjaxResponse($data, $message = '', $error = false)
    {
